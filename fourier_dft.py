    ...
    while i < len(tokens):
        ...
        if cmd in ('M','m'): #M is the absolute move, m is the relative move (move right 5 and up 10)
            ...
            while i < len(tokens) and not isinstance(tokens[i], str):
                x = tokens[i]; i += 1 #horizontal line only needs the x value
                new = cur + complex(x,0) if is_rel else complex(x, cur.imag)
                yield ('L', cur, new); cur = new
            ...
            while i+3 < len(tokens) and not isinstance(tokens[i], str):
                ...
                yield ('Q', cur, p1, p2); cur = p2

        elif cmd in ('Z','z'):
            yield ('L', cur, start_point); cur = start_point
...
path_elems = root.findall('.//{http://www.w3.org/2000/svg}path') + root.findall('.//path')
if not path_elems:
...
for p in path_elems:
    ...
    if not d:
        continue
    for seg in path_segments_from_d(d):
        segments.append(seg)
...
def segment_length(seg):
    if seg[0] == 'L':
        ...
        ts = np.linspace(0,1,80)
        pts = [quad_bezier(p0,p1,p2,t) for t in ts]
        return float(np.sum(np.abs(np.diff(pts))))
    else:
        return 0.0
...
samples_per_segment = [max(1, int(round(N_TOTAL * L / total_length))) for L in lengths]
...
while sum(samples_per_segment) != N_TOTAL:
    ...
    for i in range(len(samples_per_segment)):
        if diff == 0:
            break
        samples_per_segment[i] += 1 if diff > 0 else -1
        diff = N_TOTAL - sum(samples_per_segment)

# --- Sample points along segments ---
points = []
for seg, count in zip(segments, samples_per_segment):
    if seg[0] == 'L':
        p0, p1 = seg[1], seg[2]
